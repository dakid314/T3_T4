...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(prog='bert-ss')
    ...
    class Custom_Classifier(torch.nn.Module):
        def __init__(self, input_seq_dim, output_seq_dim):
            ...
            self.output_layer = torch.nn.Sequential(
                ...
                torch.nn.Linear(self.input_seq_dim, 128),
                torch.nn.Tanh(),
                # torch.nn.Dropout(p=0.5),
                torch.nn.Linear(128, self.output_seq_dim),
            )
...
# %%
up_task_id = 'bv3YsHosBzpnVl5RLygGGHw9zBj6EfzV'
...
upmodel.load_state_dict(torch.load(
    f"{proj_root_dir}/lib/StructurePrediction/SS/{up_task_id}/model.pt", map_location=torch.device('cpu')))
