...
import AC
def dpc(input_file, output_file):
    # 读取FASTA文件
    ...
    for seq_record in fasta_sequences:
        ...
        dac_features = [protein_id] + AC.DAC(sequence)  # 调用DAC函数计算特征
        csv_data.append(dac_features)

    # 写入CSV文件
    with open(output_file, 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(['protein_id'] + AC.dac_default_header)  # 写入CSV文件的标题行
