...
class Wangs_Model(common.Model_Final):
...
def Five_Cross_Get_model(
    ...
    for i in range(len(t_5C)):
        ...
        model: Wangs_Model = model_construct_funtion()
        model.tranmodel(
...
def research(path_dict: dict):
    ...
    t_p_f = utils.ds_preprocess.merge_pd_list([
        aac_t_p_f, CTDC_t_p_f, CTDT_t_p_f, CTDD_t_p_f, possum_t_p_f
    ])
    t_n_f = utils.ds_preprocess.merge_pd_list([
        aac_t_n_f, CTDC_t_n_f, CTDT_t_n_f, CTDD_t_n_f, possum_t_n_f
    ])
    v_p_f = utils.ds_preprocess.merge_pd_list([
        aac_v_p_f, CTDC_v_p_f, CTDT_v_p_f, CTDD_v_p_f, possum_v_p_f
    ])
    v_n_f = utils.ds_preprocess.merge_pd_list([
        aac_v_n_f, CTDC_v_n_f, CTDT_v_n_f, CTDD_v_n_f, possum_v_n_f
    ])
    ...
    Five_Cross_Get_model(
        model_construct_funtion=functools.partial(
            Wangs_Model, desc=path_dict['model']['cv']['desc'], cv=model_cv),
        t_5C=t_5C,
        v_f=v_f,
        v_l=v_l,
    ...
    Five_Cross_Get_model(
        model_construct_funtion=functools.partial(
            Wangs_Model, desc=path_dict['model']['tt']['desc'], cv=model_cv),
        t_5C=(((t_f, t_l),
               (v_f, v_l)),),
        v_f=v_f,
        v_l=v_l,
