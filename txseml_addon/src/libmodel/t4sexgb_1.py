...
class T4SEXGB_Model(common.Model_Final):
    ...
    def tranmodel(self, f, l):
        ...
        self.model = [
            ...
            SVC(gamma=0.03125, C=4, probability=True),
            MLPClassifier(
...
def research(path_dict: dict):
    ...
    aacpro_t_p_f = load_from_bert_dir_ac(
        path=path_dict['sa'].format(db_type="t_p"),
        id_list=seq_id_dict['t']['p'],
        dim=2
    )
    aacpro_t_n_f = load_from_bert_dir_ac(
        path=path_dict['sa'].format(db_type="t_n"),
        id_list=seq_id_dict['t']['n'],
        dim=2
    )
    aacpro_v_p_f = load_from_bert_dir_ac(
        path=path_dict['sa'].format(db_type="v_p"),
        id_list=seq_id_dict['v']['p'],
        dim=2
    )
    aacpro_v_n_f = load_from_bert_dir_ac(
        path=path_dict['sa'].format(db_type="v_n"),
        id_list=seq_id_dict['v']['n'],
        dim=2
    )
    ssapro_t_p_f = load_from_bert_dir_ac(
        path=path_dict['ss'].format(db_type="t_p"),
        id_list=seq_id_dict['t']['p'],
        dim=3
    )
    ssapro_t_n_f = load_from_bert_dir_ac(
        path=path_dict['ss'].format(db_type="t_n"),
        id_list=seq_id_dict['t']['n'],
        dim=3
    )
    ssapro_v_p_f = load_from_bert_dir_ac(
        path=path_dict['ss'].format(db_type="v_p"),
        id_list=seq_id_dict['v']['p'],
        dim=3
    )
    ssapro_v_n_f = load_from_bert_dir_ac(
        path=path_dict['ss'].format(db_type="v_n"),
        id_list=seq_id_dict['v']['n'],
        dim=3
    )

    disopred3_t_p_f = load_from_bert_dir_onehot(
        path=path_dict['diso'].format(db_type="t_p"),
        id_list=seq_id_dict['t']['p'],
        dim=2
    )
    disopred3_t_n_f = load_from_bert_dir_onehot(
        path=path_dict['diso'].format(db_type="t_n"),
        id_list=seq_id_dict['t']['n'],
        dim=2
    )
    disopred3_v_p_f = load_from_bert_dir_onehot(
        path=path_dict['diso'].format(db_type="v_p"),
        id_list=seq_id_dict['v']['p'],
        dim=2
    )
    disopred3_v_n_f = load_from_bert_dir_onehot(
        path=path_dict['diso'].format(db_type="v_n"),
        id_list=seq_id_dict['v']['n'],
        dim=2
    )
