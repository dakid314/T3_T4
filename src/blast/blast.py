...
# 读取阴性蛋白质序列数据
    ...
    blastp_cline = NcbiblastpCommandline(cmd='blastp', query=f"/mnt/md0/Public/T3_T4/data/new_{bac_type}/pos/{bac_type}SE.fasta", subject=f"{folder_path}/{fasta_file}", outfmt=5, out=output_file)
    blastp_cline()
    ...
    for result in blast_results:
        for alignment in result.alignments:
            for hsp in alignment.hsps:
                if hsp.identities / hsp.align_length >= 0.3 and hsp.align_length / result.query_length >= 0.7:
